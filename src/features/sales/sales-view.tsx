...
import { useSalesData } from "./sales.queries";
...
export function SalesView() {
  ...
  const {
    ...
    createProduct,
    createOrder,
  } = useSalesData(workspaceId, baseCurrency);
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // O erro fica acessível em createProduct.error
    }
  };
  ...
  const handleCreateOrder = async (e: React.FormEvent) => {
    ...
    try {
      await createOrder.mutateAsync({
        orderNumber,
        totalAmount: orderAmount ? parseFloat(orderAmount) : 0,
        paymentGateway,
        utmSource,
      });
      ...
      // O erro fica acessível em createOrder.error
    }
  };
