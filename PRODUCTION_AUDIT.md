...
* **TypeScript Check (`tsc --noEmit`):**
  * ❌ **Falha com erros de tipagem estrita:**
    1. `src/lib/action-engine.ts(22, 55, 72)`: Violação da flag `noPropertyAccessFromIndexSignature` (`proposal.payload.newBudget` / `.campaignId` ao invés de `['newBudget']` / `['campaignId']`).
    2. `src/routes/_authenticated/automations.tsx(65, 85)`: Erro de sobrecarga do cliente Supabase. A tabela `public.automations` foi criada na migração SQL `20260902030000_brain_automations_foundation.sql`, mas o arquivo estático de tipos `src/integrations/supabase/types.ts` ainda não havia incorporado as 7 tabelas da migração do Brain/Automações.
* **Vite Build (`npm run build`):**
  * Bloqueado pelos erros de TypeScript acima.
* **Dead Code / Dependências:**
  * O projeto possui `@lovable.dev/cloud-auth-js` e componentes de UI do Radix/shadcn não referenciados diretamente em páginas de produção (`aspect-ratio`, `slider`, `context-menu`).
...
1. **Estimativas Fixas em `src/routes/_authenticated/finance.tsx` (linhas 110-112):**
   ...
   ```
   * *Impacto:* Se o usuário cadastrar pedidos manuais sem cadastrar taxas reais de gateway ou custos de produto, o Costfy exibe uma DRE com números aproximados sem alertar com clareza que CMV, taxas e impostos são valores presumidos.
2. **Quebra de Canais Fixa em `src/routes/_authenticated/analytics.tsx` (linhas 60-64):**
   ...
   ```
   * *Impacto Crítico:* A distribuição por canal de mídia no Analytics não agrupa os registros da tabela `ad_metrics_daily` ou `utm_source`, aplicando uma proporção fixa fictícia (65% Meta / 35% Google / 10% Orgânico). **Isso enganaria um cliente pagante.**
3. **Sessões no Módulo de Vendas em `src/routes/_authenticated/sales.tsx` (linha 114):**
   ...
   ```
   * *Impacto:* A taxa de conversão calculada na tela de vendas usa 100 sessões fixas em vez de contar `COUNT(tracking_sessions)`.
...
1. **Correção Imediata de Integridade (P0):**
   - Corrigir os 4 erros de TypeScript em `action-engine.ts`, `automations.tsx` e sincronizar `types.ts` para liberar 100% o build de produção.
   - Substituir os números fixos/estimados de `analytics.tsx`, `finance.tsx` e `sales.tsx` por agregações reais do banco de dados (exibindo `0` ou Empty State quando não houver dados).
2. **Camada de Ingestão de Webhooks (P0):**
   - Criar endpoints/Edge Functions para Hotmart, Kiwify, Shopify e Stripe.
3. **Tracking Pixel Engine (P1):**
   - Implementar `public/track.js` e endpoint de ingestão de eventos.
